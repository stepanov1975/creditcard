...
def parse_tesseract_tsv(
    ...
    *,
    ...
    """Parse Tesseract word rows and map pixel boxes to top-left PDF points."""

    if dpi <= 0:
        raise ValueError("dpi must be positive")
    text = tsv.decode("utf-8", errors="replace") if isinstance(tsv, bytes) else tsv
    point_scale = 72.0 / dpi
    offset_x = clip[0] if clip is not None else 0.0
    offset_y = clip[1] if clip is not None else 0.0
    words: list[Word] = []
...
class TesseractOcr:
    ...
    def _render(self, path: Path, page_index: int, clip: BBox | None) -> bytes:
    ...
    def extract_words(
        ...
        if cache_path.is_file():
            ...
            tsv = self._recognize(self._render(path, page_index, clip))
            self._write_cache(cache_path, tsv)
        return parse_tesseract_tsv(tsv, dpi=self.dpi, clip=clip)
